...
import "./combat/skillmanager.js";
import { useSkill } from "./combat/skillmanager.js";
import { getTargets } from "./combat/skillmanager.js";

import "./combat/mobmanager.js"
...
function handleLevelUp(player) {
    ...
    while (expnow >= exp) {
        ...
        atkbasic = Math.floor(atkbasic * 1.005);
        defbasic = Math.floor(defbasic * 1.003);
...
world.afterEvents.itemUse.subscribe(ev => {
    ...
    // ===== 스킬 슬롯 =====
    if (id === "bridge:ironsword") {
        const target = getTargets(player);
        useSkill(player, "slash", target);
        return;
    }

        if (id === "bridge:skill_slot_1") {
        const target = getTargets(player);
        useSkill(player, "fireball", target);
        return;
    }

    if (id === "bridge:skill_slot_2") {
        const target = getTargets(player);
        useSkill(player, "fireball", target);
        return;
    }

    if (id === "bridge:skill_slot_3") {
        const target = getTargets(player);
        useSkill(player, "slash", target);
        return;
    }
...
system.runInterval(() => {
    for (const player of world.getPlayers()) {
        ...
        if (!player.hasTag("hp_inited")) {
            ...
            setScore(player, "crit_damage", 151); // 치뎀 150%
